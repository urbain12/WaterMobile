        ...
        if (my_data2[0].payment_status == "SUCCESSFUL") {
          axios.defaults.xsrfHeaderName = "X-CSRFTOKEN";
            ...
            console.log(postObj2)
              ...
              axios.post('http://admin.amazi.rw/create_order_tool/', postObj2).then((res) => {
    ...
    headers: {
      ...
      "x-auth": "705d3a96-c5d7-11ea-87d0-0242ac130003"
          ...
          axios.defaults.headers = {
            ...
            axios.post('http://admin.amazi.rw/pay_later_order_tool/create/', postObj2).then((res) => {
                console.log(res.status)
                alert('Order completed!!!')
                setpaid(true)
                clearInterval(setint)
...
setTimeout(() => {
  ...
  useEffect(()=>{
   ...
   cart.forEach(item => {
     count += item.qty;
     amount += item.qty*item.ToolID.Amount
   });
   setCartCount(count)
              ...
              <Text style={{ fontSize: 18, fontWeight: 'bold', }}></Text>
                 ...
                 {cart.map(product => (
                    <View style={{ width:'100%',height: 60, borderTopColor: '#707070', borderTopWidth: 0.5, flexDirection: 'row', padding: 10 ,alignSelf:'center'}}>
                        ...
                        <View style={{ width:'35%',alignItems:'center' }}>
                        <Text style={{ fontSize: 13, fontWeight: 'bold' }}>{product.ToolID.Title}:</Text>
                        <Text style={{ color: '#707070' }}>{product.qty} items</Text>
                        </View>

                        <View style={{ width:'25%',alignItems:'center' }}>
                        <Text style={{ fontSize: 13, fontWeight: 'bold' }}>{JSON.stringify(format(product.ToolID.Amount)).substring(1,JSON.stringify(format(product.ToolID.Amount)).length-4)} Rwf</Text>
                        </View>

                        <View style={{ width:'30%',alignItems:'center' }}>
                        <Text style={{ fontSize: 13, fontWeight: 'bold', color: "green", }}>{JSON.stringify(format(product.qty*product.ToolID.Amount)).substring(1,JSON.stringify(format(product.qty*product.ToolID.Amount)).length-4)} Rwf</Text>
                        </View>
